...
#Many issues result from assuming the grader is operating with the same data/chars/order as this notebook.
#this test program changes these defaults to try to expose any assumptions in the routine
np.random.seed(3)
_, n_a = 20, 100
test_chars = ('\n','A','B','C','D','E','F','G','H','I')
test_vocab_size = len(test_chars)

tWax, tWaa, tWya = np.random.randn(n_a, test_vocab_size), np.random.randn(n_a, n_a), np.random.randn(test_vocab_size, n_a)
tb, tby = np.random.randn(n_a, 1), np.random.randn(test_vocab_size, 1)
parameters = {"Wax": tWax, "Waa": tWaa, "Wya": tWya, "b": tb, "by": tby}

test_char_to_ix = { ch:i for i,ch in reversed (list(enumerate(test_chars))) }
test_ix_to_char = { i:ch for i,ch in reversed (list(enumerate(test_chars))) }
              
indices = sample(parameters, test_char_to_ix, 0)
print("Sampling:")
print("list of sampled indices:", indices)
print("list of sampled characters:", [test_ix_to_char[i] for i in indices])
...
# Expected Output:
# Sampling:
# list of sampled indices: [2, 2, 2, 4, 1, 3, 0]
# list of sampled characters: ['B', 'B', 'B', 'D', 'A', 'C', '\n']
