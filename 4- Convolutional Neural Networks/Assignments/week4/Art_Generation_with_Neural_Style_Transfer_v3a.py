...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.reshape(tf.transpose(a_S, perm=[0,3,1,2]), shape=[m, n_C, -1])[0]
    a_G = tf.reshape(tf.transpose(a_G, perm=[0,3,1,2]), shape=[m, n_C, -1])[0]
    ...
    print(tf.square(tf.subtract(GS, GG)))
